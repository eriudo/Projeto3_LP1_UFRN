...
/**
*@file main.cpp
*@brief Arquivo corrida que gerencia toda a parte da mecanica do jogo dos sapos.
...
void Corrida::add_arqsapo(Sapo *novo){ //adicionar o sapo no arquivo
	...
	if(z==0){
		lagoa.push_back(novo); 
		ofstream myfile ("sapinhos.txt", ios::app); /*adicionando no arquivo sapinhos.txt o novo sapo criado, caso ele nao apresente nenhum conflito, como
...
vector<Circuito*>::iterator adicionador4;
	...
	if(w == 0){
		temporada.push_back(nova); 
		ofstream myfile ("fases.txt"); //adicionando no arquivo fases.txt os circuitos criado pelo jogador
		if (myfile.is_open()){
				myfile <<  nova->getNome() << "\n";
				myfile <<  nova->getIde() << "\n";
...
void Corrida::run(){ //inicializacao do jogo
	int selecionador,aux,tamanho_corrida,random,trajetoria,salto,armazenador,posicao;
								...
								if ((*comparador2)->getDis_per() < tamanho_corrida){ /*enquanto o tamanho do iterator for menor que o tamanho maximo da corrida
								 entao a corrida continua, ou seja ela so para quando o ultimo sapo ultrapassar o tamanho maximo da corrida*/
									...
									if((*comparador2)->getDis_per() >= tamanho_corrida){
										cout << "sapo tal acabou a corrida" << (*comparador2)->getNome() << endl;
										(*comparador2)->setProvas(1 + (*comparador2)->getProvas());
										if(salto == armazenador){
				    	...
				    	if(lagoa[i]->getRanking() == 1){
				    		for (unsigned j=0; j < lagoa.size(); j++){
				    			if (lagoa[j]->getRanking() == 1 && j != i){
				    				lagoa[i]->setEmpate(1 + lagoa[i]->getEmpate());
				    			}
				    		}
				    	}	
				    }  
